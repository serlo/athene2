...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    watch: {
      ...
      svgs: {
        ...
        tasks: ['svgmin', 'clean:dist', 'copy:dist']
      },
      fonts: {
    ...
    uglify: {
      tmp: {
        ...
        files: [
          {
            '<%= serlo.tmp %>/scripts/main.js':
              '<%= serlo.tmp %>/scripts/main.js',
            '<%= serlo.tmp %>/bower_components/requirejs/require.js':
              '<%= serlo.tmp %>/bower_components/requirejs/require.js'
          }
        ]
      }
    ...
    concurrent: {
      server: ['compass', 'copy:styles', 'copy:requirejs'],
      test: ['copy:styles', 'copy:requirejs'],
      tmp: ['compass', 'copy:styles', 'copy:requirejs', 'svgmin'],
      dist: ['copy:dist']
    },
    requirejs: {
      production: {
  ...
  grunt.registerTask('test', ['requirejs:testing', 'concat:test']);

  grunt.registerTask(
    'i18n',
    'Creates an AMD module based on language files',
    function() {
  ...
  grunt.registerMultiTask(
    'language-update',
    'Fetches translation strings from source files',
    function() {
