...
define(['jquery', 'd3'], function ($, d3) {
    ...
    Birdnest = function (options) {
        ...
        this.seed = {
            i: 0,
            x1: this.calcStart({x: this.options.maxWidth / 2, y: this.options.maxHeight / 2}, 0).x,
            y1: this.calcStart({x: this.options.maxWidth / 2, y: this.options.maxHeight / 2}, 0).y,
            x2: this.calcEnd({x: this.options.maxWidth / 2, y: this.options.maxHeight / 2}, 0).x,
            y2: this.calcEnd({x: this.options.maxWidth / 2, y: this.options.maxHeight / 2}, 0).y
        };
    };
    ...
    Birdnest.prototype.calcMiddle = function (lastLine, scale) {
        var i = lastLine.i + 1,
            x = this.options.maxWidth / 2 + (Math.min(this.options.maxWidth, this.options.maxHeight) - this.options.lineLength) / 2 *
                i * Math.cos(2 * Math.PI * i / this.options.stepwidth) / scale,
            y = this.options.maxHeight / 2 + (Math.min(this.options.maxWidth, this.options.maxHeight) - this.options.lineLength) / 2 *
                i * Math.sin(2 * Math.PI * i / this.options.stepwidth) / scale;
    ...
    $.fn.renderNest = function () {
        $(this).each(function () {
            ...
            $(this).empty().append(
                birdnest.createSVG($(this).data('amount'))
            );
        });
    };
});
